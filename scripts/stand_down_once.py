#!/usr/bin/env python3
"""Send one guarded StandDown request after physical-area confirmation."""
...
from go2.motion import MotionError, OneShotStandDownController  # noqa: E402
...
async def run(config_path: str | None, confirmed: bool) -> None:
    ...
    try:
        ...
        print("GUARDED_STAND_DOWN=PASS")
        print(
            f"Commands: StandDown status={result.stand_status}; "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="One guarded GO2 Air StandDown action; this physically moves the robot."
    )
    parser.add_argument("--config", help="Path to YAML configuration")
    parser.add_argument(
        "--confirm-clearance",
        action="store_true",
        help="Confirm people and objects remain clear of the robot",
    )
...
def main() -> int:
    ...
    try:
        ...
        print(f"Guarded StandDown failed: {exc}", file=sys.stderr)
        return 1
